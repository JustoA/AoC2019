...
public class Main {
	...
	public static void main(String[] args) {
		...
		// Input in = new
		// Input("2,9,27,31,1,5,31,35,1,35,10,39,1,39,10,43,2,43,9,47,1,6,47,51,2,51,6,55,1,5,55,59,2,59,10,63,1,9,63,67,1,9,67,71,2,71,6,75,1,5,75,79,1,5,79,83,1,9,83,87,2,87,10,91,2,10,91,95,1,95,9,99,2,99,9,103,2,10,103,107,2,9,107,111,1,111,5,115,1,115,2,119,1,119,6,0,99,2,0,14,0");
		Input wire1 = new Input(
		...
		//Input wire1 = new Input("R98,U47,R26,D63,R33,U87,L62,D20,R33,U53,R51");
		//Input wire2 = new Input("U98,R91,D20,R16,D67,R40,U7,R15,U6,R7");
		ArrayList<String> matches = new ArrayList<String>();
		// this is going to be gross.
		int[][] box = new int[30000][30000]; // i hope this is enough.
		...
		i = 15000;
		j = 15000;
		for (String s : wire2.inList) {
			char dir = s.charAt(0);
			int dist = Integer.valueOf(s.substring(1));
			if (dir == 'L') {
				for (; dist != 0; i--, dist--) {
					if (box[i][j] == 1) {// System.out.println("INTERSECTION "+i+","+j);
						matches.add(i + "," + j);
						box[i][j] = 3;
					}
				}
		...
		for (String s : matches) {
			...
			distances.add(Math.abs(x - 15000) + Math.abs(y - 15000));
		}
		Collections.sort(distances);
		System.out.println("Distance to closest intersection: "+distances.get(1));

		/// PART 2
		i = 15000;
		j = 15000;
		HashMap<String, Integer> inters1 = new HashMap<String, Integer>();
		int wireOneSteps = 0;
		for (String s : wire1.inList) {
			...
			if (dir == 'L') {
				for (; dist != 0; i--, dist--) {
					if (box[i][j] == 3 && !inters1.containsKey(i + "," + j)) {
						inters1.put(i + "," + j, wireOneSteps);
					}
					wireOneSteps++;
				}
			}
		...
		for (String s : wire2.inList) {
			...
			if (dir == 'L') {
				for (; dist != 0; i--, dist--) {
					...
					if (box[i][j] == 3) {
						inters1.put(i + "," + j, inters1.get(i+","+j)+wireTwoSteps);
						box[i][j] = 9;
					}
					wireTwoSteps++;
				}
		...
		dists.addAll(inters1.values());
		Collections.sort(dists);
		//System.out.println(dists.get(0));
		System.out.println("Shortest steps for both wires: " + dists.get(1));
	}
}
